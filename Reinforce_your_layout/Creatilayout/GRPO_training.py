...
from training_patch.pipeline.sd3_grpo_pipeline_with_logprob import sd3_pipeline_with_logprob
from training_patch.logprob.sd3_dspo_sde_with_logprob import sde_step_with_logprob
...
def _reload_for_sampling(pipeline, device, inf_dtype):
    pipeline.vae.to(device, dtype=torch.float32)
    pipeline.text_encoder.to(device, dtype=inf_dtype)
    pipeline.text_encoder_2.to(device, dtype=inf_dtype)
    pipeline.text_encoder_3.to(device, dtype=inf_dtype)

def _offload_for_training(pipeline):
    pipeline.vae.to("cpu")
...
def compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, layout_kwargs, bbox_scale, noise_level):
...
def main(_):
    ...
    if accelerator.is_main_process:
        accelerator.init_trackers(
            project_name="fine-tune-sd3-grpo",
    ...
    ema = EMAModuleWrapper(transformer_trainable_parameters, decay=0.9, update_step_interval=8, device="cpu")
    ...
    while True:
        ...
        _reload_for_sampling(pipeline, accelerator.device, inference_dtype)
        #################### SAMPLING ####################
        pipeline.transformer.eval()
        samples = []
        for i in tqdm(
            ...
            with autocast():
                with torch.no_grad():
                    ...
                    for idx in range(len(global_captions)):
                        ...
                        latents_traj_list.append(latents_i)       # [1, C_lat, H_lat, W_lat]
            ...
            images = torch.cat(images_list, dim=0).to(torch.float32).cpu()                   # [B, 3, H, W]
            log_probs_list = [torch.stack(x, 0).unsqueeze(0) if isinstance(x, (list, tuple)) else x.reshape(1, -1) for x in log_probs_list]
            log_probs = torch.cat(log_probs_list, dim=0).to(torch.float32).cpu() 
            prompt_embeds = torch.cat(prompt_embeds_list, dim=0).to(torch.float32).cpu()     # [B, L, D_attn]
            pooled_prompt_embeds = torch.cat(pooled_prompt_embeds_list, dim=0).to(torch.float32).cpu()  # [B, D_pool]
            boxes = torch.cat(boxes_list, dim=0).to(torch.float32).cpu()                 # [B, MAX_REGIONS, 4]
            text_embeddings = torch.cat(text_embeds_list, dim=0).to(torch.float32).cpu() # [B, MAX_REGIONS, D_txt]
            masks = torch.cat(masks_list, dim=0).to(torch.float32).cpu()                 # [B, MAX_REGIONS, Hm, Wm]

            B = images.shape[0]
            T = log_probs.shape[1]
            ts_dtype = pipeline.scheduler.timesteps.dtype
            ts_rows = []
            for ts in ret_ts_list:
                t = torch.as_tensor(ts, device="cpu").view(-1)
                if t.numel() >= T:
                    t = t[:T]
                else:
                    t = torch.cat([t, t[-1:].repeat(T - t.numel())], dim=0)
                ts_rows.append(t)

            timesteps = torch.stack(ts_rows, dim=0).to(torch.float32).cpu() 

            traj_tensors = []
            for i in range(B):
                traj_i_list = _as_traj_list(latents_traj_list[i])
                traj_i_list = [t.to(torch.float32).cpu() for t in traj_i_list]

                if len(traj_i_list) == T + 1:
                    pass
                elif len(traj_i_list) == 1:
                    traj_i_list = traj_i_list * (T + 1)
                else:
                    if len(traj_i_list) > T + 1:
                        traj_i_list = traj_i_list[: T + 1]
                    else:
                        traj_i_list = traj_i_list + [traj_i_list[-1]] * (T + 1 - len(traj_i_list))

                traj_i = torch.stack(traj_i_list, dim=0).squeeze(1)  
                traj_tensors.append(traj_i)

            latents_traj = torch.stack(traj_tensors, dim=0)  
            latents = latents_traj[:, :-1, ...]      # [B, T, C, H_lat, W_lat]
            next_latents = latents_traj[:, 1:, ...]  # [B, T, C, H_lat, W_lat]
        ...
        _offload_for_training(pipeline)
        # #################### TRAINING ####################
        for inner_epoch in range(config.train.num_inner_epochs):
            ...
            for i, sample in tqdm(
                ...
                sample["latents"]      = sample["latents"].to(accelerator.device, non_blocking=True)
                sample["timesteps"]    = sample["timesteps"].to(accelerator.device, non_blocking=True)
                sample["next_latents"] = sample["next_latents"].to(accelerator.device, non_blocking=True)
                sample["log_probs"]    = sample["log_probs"].to(accelerator.device, non_blocking=True)
                sample["advantages"]   = sample["advantages"].to(accelerator.device, non_blocking=True)

                embeds = sample["prompt_embeds"].to(accelerator.device, non_blocking=True)
                pooled_embeds = sample["pooled_prompt_embeds"].to(accelerator.device, non_blocking=True)
                boxes = sample["boxes"].to(accelerator.device, non_blocking=True)
                text_embeddings = sample["text_embeddings"].to(accelerator.device, non_blocking=True)
                masks = sample["masks"].to(accelerator.device, non_blocking=True)
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        ...
                        prev_sample, log_prob, prev_sample_mean, std_dev_t = compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, layout_kwargs, bbox_scale, config.sample.s_churn)
                        if config.train.beta > 0:
                                with torch.no_grad():
                                    with transformer.module.disable_adapter():
                                        _, _, prev_sample_mean_ref, _ = compute_log_prob(transformer, pipeline, sample, j, embeds, pooled_embeds, layout_kwargs, bbox_scale, config.sample.s_churn)
