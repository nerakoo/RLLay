...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        lora_processor: Optional["AttnProcessor"] = None,
        out_dim: int = None,
        ...
        self.proj_lora_img = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #img attn.proj
        self.lora_weight = lora_weight

        #img
        self.img_q_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for img q in img-bbox attn
        self.img_k_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for img k in img-bbox attn
        self.img_v_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for img v in img-bbox attn

        #bbox
        self.bbox_q_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for bbox q in img-bbox attn
        self.bbox_k_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for bbox k in img-bbox attn
        self.bbox_v_lora = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha) #lora for bbox v in img-bbox attn

        if self.context_pre_only is not None and not self.context_pre_only:
            self.proj_lora_bbox = LoRALinearLayer(self.inner_dim, self.inner_dim, rank, network_alpha)
        ...
        self.set_processor(processor,lora_processor)
    ...
    def set_processor(self, processor: "AttnProcessor",lora_processor: "AttnProcessor") -> None:
        r"""
        ...
        self.lora_processor = lora_processor
    ...
    def forward(
        ...
        bbox_hidden_states: Optional[torch.Tensor] = None,
        is_lora_processor: bool = False,
...
class JointAttnProcessor2_0:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # set_trace()
        input_ndim = hidden_states.ndim # torch.Size([B, 4096, 1536])
        if input_ndim == 4:
            batch_size, channel, height, width = hidden_states.shape
            hidden_states = hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        context_input_ndim = encoder_hidden_states.ndim #torch.Size([2, 333, 1536])
        ...
        # attn_weight_map = attn_weight[-1].mean(dim=0)
        # attn_weight_softmax = torch.softmax(attn_weight, dim=-1)
        # attn_weight_softmax_map = attn_weight_softmax[-1].mean(dim=0)
        # from IPython.core.debugger import set_trace 
        # set_trace()
        ...
        return hidden_states, encoder_hidden_states # torch.Size([B, 4096, 1536]), torch.Size([B, 333, 1536])
...
class LoRAJointAttnProcessor2_0:
    """Attention processor used typically in processing the SD3-like self-attention projections."""
    ...
    def __call__(
        ...
        attn: Attention,
        hidden_states: torch.FloatTensor,
        bbox_hidden_states: torch.FloatTensor = None,
        attention_mask: Optional[torch.FloatTensor] = None,
        *args,
        **kwargs,
    ) -> torch.FloatTensor:
        residual = hidden_states
        residual_bbox = bbox_hidden_states
        # from IPython.core.debugger import set_trace 
        # set_trace()
        input_ndim = hidden_states.ndim # torch.Size([B, 4096, 1536])
        if input_ndim == 4:
            batch_size, channel, height, width = hidden_states.shape
            hidden_states = hidden_states.view(batch_size, channel, height * width).transpose(1, 2)
        bbox_input_ndim = bbox_hidden_states.ndim #torch.Size([2, x, 1536])
        if bbox_input_ndim == 4:
            batch_size, channel, height, width = bbox_hidden_states.shape
            bbox_hidden_states = bbox_hidden_states.view(batch_size, channel, height * width).transpose(1, 2)

        batch_size = bbox_hidden_states.shape[0]


        # `sample` projections.
        query = attn.to_q(hidden_states) + attn.lora_weight * attn.img_q_lora(hidden_states)
        key = attn.to_k(hidden_states) + attn.lora_weight * attn.img_k_lora(hidden_states)
        value = attn.to_v(hidden_states) + attn.lora_weight * attn.img_v_lora(hidden_states)
       
        
        # `bbox` projections.
        bbox_hidden_states_query_proj = attn.add_q_proj(bbox_hidden_states) + attn.lora_weight * attn.bbox_q_lora(bbox_hidden_states)
        bbox_hidden_states_key_proj = attn.add_k_proj(bbox_hidden_states) + attn.lora_weight * attn.bbox_k_lora(bbox_hidden_states)
        bbox_hidden_states_value_proj = attn.add_v_proj(bbox_hidden_states) + attn.lora_weight * attn.bbox_v_lora(bbox_hidden_states)

       
        query = torch.cat([query, bbox_hidden_states_query_proj], dim=1)
        key = torch.cat([key, bbox_hidden_states_key_proj], dim=1)
        value = torch.cat([value, bbox_hidden_states_value_proj], dim=1)
        ...
        hidden_states, bbox_hidden_states = (
            hidden_states[:, : residual.shape[1]],
            hidden_states[:, residual.shape[1] :],
        )
    
        # linear proj with lora
        hidden_states = attn.to_out[0](hidden_states) + attn.lora_weight * attn.proj_lora_img(hidden_states)
     
        # dropout
        hidden_states = attn.to_out[1](hidden_states)
        if not attn.context_pre_only:
            bbox_hidden_states = attn.to_add_out(bbox_hidden_states) + attn.lora_weight * attn.proj_lora_bbox(bbox_hidden_states)

        if input_ndim == 4:
            hidden_states = hidden_states.transpose(-1, -2).reshape(batch_size, channel, height, width)
        if bbox_input_ndim == 4:
            bbox_hidden_states = bbox_hidden_states.transpose(-1, -2).reshape(batch_size, channel, height, width)
