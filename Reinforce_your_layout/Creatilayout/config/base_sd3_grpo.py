...
def get_config():
    ...
    sample.s_churn = 0.9 # control the intensity of additional noise added during sampling
    sample.train_batch_size = 4
    sample.num_image_per_prompt = 4
    # sample.num_batches_per_epoch = int(32/(gpu_number*config.sample.train_batch_size/config.sample.num_image_per_prompt)) # number of batches to sample per epoch. the total number of samples per epoch is `num_batches_per_epoch * batch_size * num_gpus`.
    ...
    train.gradient_accumulation_steps = config.sample.num_batches_per_epoch//2 # number of gradient accumulation steps. the effective batch size is `batch_size * num_gpus * gradient_accumulation_steps`.
